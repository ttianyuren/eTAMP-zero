...
def multi_rrt(start, goal, distance_fn, sample_fn, extend_fn, collision_fn, 
              max_time=INF, max_iterations=INF, goal_probability=.2, max_tree_size=500):
    """
    ...
    :param goal_probability: Probability of biasing sampling towards the goal
    :param max_tree_size: Maximum size of the tree
    ...
    # Initialize the MultiRRT tree
    tree = MultiRRT(start, distance_fn, sample_fn, extend_fn, collision_fn)

    # Track timing and iterations
    start_time = time()
    iteration = 0

    # Main loop for growing the RRT
    while elapsed_time(start_time) < max_time and iteration < max_iterations:
        if iteration % PRINT_FREQUENCY == 0:
            print('Iteration: {} | Time: {:.3f} | Nodes: {}'.format(
                iteration, elapsed_time(start_time), len(tree)))

        # Grow the tree and attempt to reach the goal
        path = tree.grow(goal, iterations=1, goal_probability=goal_probability, max_tree_size=max_tree_size)

        # If a path is found to the goal, return it
        if path is not None:
            return path

        iteration += 1

    # Return None if no path was found within the time/iteration limit
    return None
